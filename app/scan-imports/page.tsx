...
export default function ScanImportsPage() { /* omitted for brevity */
  ...
  const parseScan = async (file?: File) => {
    ...
    const { data: createdImport, error: importError } = await supabase.from('imports').insert({ organization_id: orgId, project_id: projectId, source_type: 'scan', file_name: file.name, status: 'parsed' }).select('id').single();
